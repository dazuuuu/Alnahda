...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saudiaOnlyFields = document.querySelectorAll(".saudia-only");
  const willingLabel = document.getElementById("willingLabel");

  function toggleSaudiaSections(value) {
    if (value === "yes") {
      saudiaOnlyFields.forEach((f) => (f.style.display = "block"));
      willingLabel.textContent = "Are you willing to go back to SAUDIA?";
    } else {
      saudiaOnlyFields.forEach((f) => (f.style.display = "none"));
      willingLabel.textContent = "Are you willing to go to SAUDIA?";
    }
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (window.crypto?.randomUUID) {
        fd.set("id", window.crypto.randomUUID());
      }
      ...
      const ok = res.ok && (text.includes("✅") || /success/i.test(text));
      if (ok) {
        setFeedback("success", "✅ Application submitted successfully!");
        form.reset();
        if (submitButton) submitButton.disabled = false;
      } else {
        setFeedback("error", text || "We could not save your application. Please try again.");
        if (submitButton) submitButton.disabled = false;
      }
    } catch (err) {
      console.error(err);
